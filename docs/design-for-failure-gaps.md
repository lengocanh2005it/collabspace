...
Dùng thư viện `opossum` (Node.js circuit breaker phổ biến nhất):
```typescript
import CircuitBreaker from 'opossum';

const breaker = new CircuitBreaker(fetchMembership, {
  timeout: 3000,           // timeout mỗi call
  errorThresholdPercentage: 50,  // 50% lỗi → OPEN
  resetTimeout: 30000,     // thử lại sau 30s
});

breaker.fallback(() => null); // fallback khi OPEN
```
...
**Giải pháp:**  
Khi workspace-service phát sự kiện `workspace_member_removed` lên Kafka, task-service lắng nghe và gọi `membershipCache.clear(workspaceId, userId)`. Hiện tại task-service đã có consumer lắng nghe `workspace_deleted` — chỉ cần thêm handler cho `member_removed`.
...
|-----|--------|---------------|------------------------------|
| Circuit Breaker | Cao | Trung bình (cần lib `opossum`) | ✅ Nên làm |
| Retry HTTP sync | Trung bình | Thấp (vài dòng code) | ✅ Nên làm |
| Rate Limiting | Trung bình | Thấp (`@nestjs/throttler`) | ✅ Nên làm |
| Bulkhead | Thấp | Cao (Node.js không support tốt) | ⚠️ Biết nhưng chấp nhận |
| Cache invalidation | Cao | Thấp (thêm Kafka consumer handler) | ✅ Nên làm |
