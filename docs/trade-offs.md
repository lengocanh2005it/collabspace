...
| **B3** *(removed)* | Legacy shared static token S2S | Superseded by B3.1 |
| **B3.1** Service JWT (HTTP internal) | task→workspace membership, replica fallback | `Authorization: Bearer` + scope/`iss`/`aud`; symmetric `SERVICE_JWT_SECRET`; contract — [service-contracts.md](../.claude/docs/service-contracts.md#service-to-service-http-authentication-service-jwt) |
