...
| CI/CD pipeline (GitHub Actions) | ✅ CI + GHCR build + Helm deploy on `main` + post-deploy `demo-e2e` smoke |
...
| Demo E2E script 7 bước | ✅ `scripts/demo-e2e.*` + `run-demo-e2e-prod.sh`; gate CI smoke sau deploy `main` |
