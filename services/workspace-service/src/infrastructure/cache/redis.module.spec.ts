...
describe('workspace-service RedisModule', () => {
  ...
  it('creates standalone client with host/port using ws: prefix', async () => {
    await createClient({ REDIS_HOST: 'localhost', REDIS_PORT: '6380' });
    expect(MockRedis).toHaveBeenCalledWith(
      expect.objectContaining({ host: 'localhost', port: 6380, keyPrefix: 'ws:' }),
    );
  });
  ...
  it('creates standalone client with REDIS_URL', async () => {
    ...
    expect(MockRedis).toHaveBeenCalledWith(
      ...
      expect.objectContaining({ keyPrefix: 'ws:' }),
    );
  });
