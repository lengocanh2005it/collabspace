...
const MockRedis: jest.Mock = jest.requireMock("ioredis").default;
...
describe("notification-service RedisModule", () => {
  ...
  it("creates standalone client with notif: prefix", async () => {
    await createClient({ REDIS_HOST: "localhost", REDIS_PORT: "6379" });
    expect(MockRedis).toHaveBeenCalledWith(
      expect.objectContaining({ host: "localhost", port: 6379, keyPrefix: "notif:" }),
    );
  });
  ...
  it("creates standalone client with REDIS_URL", async () => {
    ...
    expect(MockRedis).toHaveBeenCalledWith(
      ...
      expect.objectContaining({ keyPrefix: "notif:" }),
    );
  });

  it("creates sentinel client without keyPrefix for pub/sub compatibility", async () => {
