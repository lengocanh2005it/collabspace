...
export class DlqHealthService {
  ...
  async getReadiness(): Promise<ReadinessReport> {
    ...
    const checks: Record<string, HealthCheckResult> = {
      ...
      kafka: kafkaConfig.enabled
        ? {
            detail: `Kafka consumer enabled (${kafkaConfig.brokers.join(',')})`,
            required: false,
            status: 'up',
          }
        : {
            detail: 'Kafka consumer disabled (KAFKA_CONSUMERS_ENABLED=false)',
            required: false,
            status: 'disabled',
          },
    };

    return this.toReadinessReport('dlq-service', checks);
  }
  ...
  private async runCheck(
    ...
    try {
      await operation();
      return { required, responseTimeMs: Date.now() - startedAt, status: 'up' };
    } catch (error) {
      return {
