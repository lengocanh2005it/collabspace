...
export class SlimUserProfiles1718000000102 implements MigrationInterface {
  ...
  async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(`ALTER TABLE profiles DROP COLUMN IF EXISTS cover_url`);
    await queryRunner.query(`ALTER TABLE profiles DROP COLUMN IF EXISTS job_title`);
    await queryRunner.query(`ALTER TABLE profiles DROP COLUMN IF EXISTS department`);
    await queryRunner.query(`ALTER TABLE profiles DROP COLUMN IF EXISTS location`);
    await queryRunner.query(`ALTER TABLE profiles DROP COLUMN IF EXISTS timezone`);
    await queryRunner.query(`ALTER TABLE profiles DROP COLUMN IF EXISTS locale`);
    await queryRunner.query(`ALTER TABLE profiles DROP COLUMN IF EXISTS email_verified`);
  }
