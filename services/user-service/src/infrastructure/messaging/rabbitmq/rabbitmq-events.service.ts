...
import { 
  USER_REGISTERED_EVENT, UserRegisteredEventPayload
} from '../../../domain/events/user-create.event';
import { USER_PROFILE_UPDATED_EVENT, UserProfileUpdatedEventPayload } from '../../../domain/events/user-profile-update.event';
...
export class RabbitMqEventsService implements OnModuleDestroy {
  ...
  private getTaskClient(): ClientProxy {
    ...
    this.taskClient = ClientProxyFactory.create({
      ...
      options: { urls: [url], queue: 'task-service', queueOptions: { durable: true } },
    });
  ...
  private getNotiClient(): ClientProxy {
    ...
    this.notiClient = ClientProxyFactory.create({
      transport: Transport.RMQ,
      options: { urls: [url], queue: 'notification-service', queueOptions: { durable: true } },
    });
