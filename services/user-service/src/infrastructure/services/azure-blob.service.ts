...
export class AzureBlobService {
  ...
  constructor(private readonly configService: ConfigService) {
    ...
    if (!connectionString) {
      ...
      this.logger.warn("⚠️ Azure Blob Storage not configured - running in MOCK MODE (file uploads not persisted)");
      return;
    }
    ...
    try {
      ...
      this.isMockMode = true;
      this.logger.error(`Failed to initialize Azure Blob Storage client: ${error instanceof Error ? error.message : String(error)}. Falling back to MOCK MODE.`);
    }
  }
  ...
  async onModuleInit() {
    ...
    try {
      ...
      this.logger.error(`Failed to initialize container: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
  ...
  async uploadAvatar(file: UploadedFile, userId: string): Promise<string> {
    try {
      ...
      await blockBlobClient.uploadData(file.buffer, {
        ...
        metadata: {
          userId: userId,
          originalName: Buffer.from(file.originalname).toString("ascii").replace(/[^a-zA-Z0-9.-]/g, "_"),
