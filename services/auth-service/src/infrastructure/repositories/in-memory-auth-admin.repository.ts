...
export class InMemoryAuthAdminRepository implements AuthAdminRepository {
  ...
  async createRole(input: { description: string; name: string }): Promise<AdminRole> {
    ...
    if (this.roles.some((role) => role.name === name)) {
      throw new ConflictException({ code: 'ROLE_ALREADY_EXISTS', message: `Role ${name} already exists` });
    }
    const role = { description: input.description.trim(), id: randomUUID(), name, permissions: [] };
  ...
  async createPermission(input: { description: string; name: string }): Promise<AdminPermission> {
    ...
    if (this.permissions.some((permission) => permission.name === name)) {
      throw new ConflictException({ code: 'PERMISSION_ALREADY_EXISTS', message: `Permission ${name} already exists` });
    }
    const permission = { description: input.description.trim(), id: randomUUID(), name };
  ...
  async assignPermissionToRole(roleId: string, permissionId: string): Promise<AdminRole> {
    ...
    if (!permission) throw new NotFoundException({ code: 'PERMISSION_NOT_FOUND', message: `Permission ${permissionId} was not found` });
  ...
  async listRoles(): Promise<AdminRole[]> { return [...this.roles]; }
  async listPermissions(): Promise<AdminPermission[]> { return [...this.permissions]; }
  ...
  async updateRole(roleId: string, input: { description?: string; name?: string }): Promise<AdminRole> {
    const role = this.requireRole(roleId);
  ...
  async deleteRole(roleId: string): Promise<void> {
    ...
    if (index < 0) throw new NotFoundException({ code: 'ROLE_NOT_FOUND', message: `Role ${roleId} was not found` });
    const role = this.roles[index];
    if (['admin', 'member', 'viewer'].includes(role.name) || this.users.some((user) => user.roles.includes(role.name))) {
      throw new ConflictException({ code: 'ROLE_IN_USE', message: `Role ${role.name} cannot be deleted while protected or assigned` });
    }
  ...
  private requireRole(id: string): AdminRole {
    ...
    if (!role) throw new NotFoundException({ code: 'ROLE_NOT_FOUND', message: `Role ${id} was not found` });
    return role;
  }

  private requireUser(id: string): AdminUser {
    const user = this.users.find((item) => item.id === id);
    if (!user) throw new NotFoundException({ code: 'USER_NOT_FOUND', message: `User ${id} was not found` });
    return user;
  }
}
