...
describe('CreateTaskHandler', () => {
  ...
  it('should create a task successfully when creator exists and is active', async () => {
    ...
    const savedTask = mockTaskRepo.addAsync.mock.calls[0][0] as Task;
  ...
  it('should throw BadRequestException if creator does not exist', async () => {
    ...
    await expect(handler.execute(command)).rejects.toThrow('Tài khoản người tạo Task không tồn tại hoặc đã bị khóa!');
