...
describe('CreateCommentHandler', () => {
  ...
  it('should create comment and emit event if task has an assignee different from author', async () => {
    const command = new CreateCommentCommand('123e4567-e89b-12d3-a456-426614174000', 'author-1', 'Great task!');
    ...
    expect(mockRabbitMqEvents.publishTaskCommented).toHaveBeenCalledWith(expect.objectContaining({
  ...
  it('should create comment but NOT emit event if author IS the assignee', async () => {
    const command = new CreateCommentCommand('123e4567-e89b-12d3-a456-426614174000', 'author-1', 'Self note');
