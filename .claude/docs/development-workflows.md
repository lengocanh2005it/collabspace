...
**k3s production (Droplet):** after CI images include `seed:prod` and the app rollout is healthy, run `bash infrastructure/deploy/run-k8s-seed.sh` on the server. The seed script uses the currently deployed image tag by default; set `IMAGE_TAG=<sha>` only when intentionally seeding a different image. E2E without Resend: `BASE_URL=http://<host>/api/v1 bash infrastructure/deploy/run-demo-e2e-prod.sh` (OTP via `read-auth-otp-from-outbox.sh`). With Resend: `bash infrastructure/deploy/configure-prod-resend.sh` after filling `phase0.env`.
