...
4. DB migration Jobs chỉ chạy khi có file migration đổi trong `services/{auth,user,workspace}-service/migrations/**` hoặc `src/migrations/**`; manual dispatch vẫn chạy migration để an toàn.
5. Service **không** đổi giữ nguyên tag trong `values-prod.yaml`.
